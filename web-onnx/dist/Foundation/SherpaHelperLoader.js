...
// Generic Loader
// ---------------------------------------------------------------------------
async function loadSherpaModule(filename, exportNames) {
    const cached = moduleCache.get(filename);
    if (cached)
        return cached;
    const promise = doLoad(filename, exportNames);
...
async function doLoad(filename, exportNames) {
    ...
    if (!response.ok) {
        throw new SDKError(SDKErrorCode.WASMLoadFailed, `Failed to fetch ${filename}: ${response.status} ${response.statusText}`);
    }
    ...
    try {
        ...
        throw new SDKError(SDKErrorCode.WASMLoadFailed, `Failed to load sherpa helper ${filename}: ${msg}`);
    }
    finally {
...
export function loadASRHelpers() {
    return loadSherpaModule('sherpa-onnx-asr.js', ASR_EXPORTS);
}
/** Load TTS struct-packing helpers (sherpa-onnx-tts.js). */
export function loadTTSHelpers() {
    return loadSherpaModule('sherpa-onnx-tts.js', TTS_EXPORTS);
}
/** Load VAD struct-packing helpers (sherpa-onnx-vad.js). */
export function loadVADHelpers() {
    return loadSherpaModule('sherpa-onnx-vad.js', VAD_EXPORTS);
}
//# sourceMappingURL=SherpaHelperLoader.js.map