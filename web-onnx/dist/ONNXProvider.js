 ...
 */
import { SDKLogger, ModelManager, ExtensionPoint, BackendCapability, ExtensionRegistry, extractTarGz, } from '@runanywhere/web';
...
async function loadSTTFromArchive(ctx, sherpa, modelDir) {
...
async function loadSTTFromIndividualFiles(ctx, sherpa, modelDir) {
...
async function loadTTSFromArchive(ctx, sherpa, modelDir) {
    ...
    if (!modelPath)
        throw new Error(`TTS archive for '${ctx.model.id}' does not contain an .onnx model file`);
    if (!tokensPath)
        throw new Error(`TTS archive for '${ctx.model.id}' does not contain tokens.txt`);
    await TTS.loadVoice({
...
async function loadTTSFromIndividualFiles(ctx, sherpa, modelDir) {
    const primaryFilename = ctx.model.url.split('/').pop();
    const primaryPath = `${modelDir}/${primaryFilename}`;
    sherpa.writeFile(primaryPath, ctx.data);
    const additionalPaths = {};
...
// Helper
// ---------------------------------------------------------------------------
function findArchivePrefix(paths) {
    if (paths.length === 0)
        return '';
...
//# sourceMappingURL=ONNXProvider.js.map